...
plt.ylim(2000,5000)
#plt.title("Comparison of vaccine delivery methods\n(500 simulations each)")
...
for i in range(0, 24):
    labl= teamStrat[i]    
    #plt.scatter(combinedCosts[i], combinedDeaths[i], label = "1 UAV + 1 vehicle", s=12,c='steelblue')
    #plt.scatter(droneCosts[i], droneDeaths[i], label = "2 UAVs", s=12, c='gold')
    plt.scatter(vehicleCosts[i],vehicleDeaths[i], label = "2 vehicles", s=12,c='mediumseagreen')


plt.scatter(combinedCosts[:], combinedDeaths[:], label = "1 UAV + 1 vehicle", s=12,c='steelblue')
plt.scatter(droneCosts[:], droneDeaths[:], label = "2 UAVs", s=12, c='gold')
plt.scatter(vehicleCosts[:],vehicleDeaths[:], label = "2 vehicles", s=12,c='mediumseagreen')
